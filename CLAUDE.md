...
- `test/` — `npm test`: black-box installer tests + asset-tree consistency (node:test, no deps, no network). Runs on every PR. Language jalons (`test/rust-gates.test.mjs`, `test/python-gates.test.mjs`, `test/go-gates.test.mjs`) skip when their toolchain is missing; the matching CI jobs install it and set `RUST_GATES=1` / `PYTHON_GATES=1` / `GO_GATES=1` so a skip cannot pass.
