...
- `workstation/` — the operator's own tools (`fleet`, `bench`, zellij layouts), reachable from NO registry entry and installed to the machine by `claude-rules workstation install`, never into a repo. A layout is not path-scopable and a cross-repo dashboard cannot be a per-repo copy; what the library owns here is the file contract these read (`.work/<slug>/loop.md`, `## Blocked on the human`), not the reader
